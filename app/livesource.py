...
This module no longer decides whether an aircraft belongs to a leg. In v4
`live_state_for_leg` lived here and both the poller AND every page render
called it, so a family member opening the app advanced the aircraft state
machine and wrote track points. That orchestration now belongs to the
poller alone (see poller.py); this is a pure fetch-and-cache.
"""
