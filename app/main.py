...
async def viewer_settings_get(request: Request):
    ...
    # A viewer has no aeroapi_* or poll_seconds of their own, so those
    # attributes are simply absent here and every use of them in the
    # template sits behind is_pilot. settings_previews reads them with
    # getattr defaults for the same reason.
    ...
    return HTMLResponse(template.render(
        request=request, s=view_settings,
        is_pilot=False, is_admin=False, active_tab="settings",
        post_to="/viewer-settings", accents=ACCENTS,
        preview=settings_previews(view_settings),
        notice=request.session.pop("_settings_notice", ""),
        error="", open_group="", account=None,
    ))
...
async def viewer_settings_post(
    ...
    resp.set_cookie("pt_viewer_accent",
                    accent if accent in ACCENTS else DEFAULT_ACCENT,
...
def _settings_context(request: Request, pilot) -> dict:
    """Everything settings.html needs for a pilot, built in one place.

    The GET route and all three POST routes render the same page, and the
    context drifting between them is how a form comes back missing its
    usage figure — which is a bug that looks exactly like the API key
    having been wiped.
    """
