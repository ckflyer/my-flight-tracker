...
from app.flights import (get_row, save_schedule, write, load_schedule,  # noqa: E402
                         purge_old, flight_key)
...
print("\n-- write modes --")
write(UID, LEG.id, once={"off_observed": "2026-08-04T19:10:00+00:00"})
write(UID, LEG.id, once={"off_observed": "2026-08-04T19:40:00+00:00"})
check("ONCE keeps the first value",
      get_row(UID, LEG.id)["off_observed"].startswith("2026-08-04T19:10"))

write(UID, LEG.id, latest={"last_speed_kts": 420})
write(UID, LEG.id, latest={"last_speed_kts": None})
check("LATEST: a blank never overwrites a known value",
      get_row(UID, LEG.id)["last_speed_kts"] == 420)
write(UID, LEG.id, latest={"last_speed_kts": 380})
check("LATEST: a real value does overwrite",
      get_row(UID, LEG.id)["last_speed_kts"] == 380)

write(UID, LEG.id, always={"progress_pct": 44.0})
write(UID, LEG.id, always={"progress_pct": None})
check("ALWAYS overwrites even with nothing",
      get_row(UID, LEG.id)["progress_pct"] is None)


print("\n-- schedule re-import keeps observed data --")
save_schedule(UID, [LEG])
check("re-pasting the same schedule keeps the observed wheels-off",
      get_row(UID, LEG.id)["off_observed"] is not None)
...
print("\n-- phase from the row --")
write(UID, LEG.id, always={"last_on_ground": 0, "last_lat": 33.0, "last_lon": -97.5,
                           "airborne_seen": 1})
check("airborne far out reads In air",
      tags.compute_phase(get_row(UID, LEG.id), LEG, DEP) == tags.PHASE_IN_AIR)

d = LEG.dest_info
write(UID, LEG.id, always={"last_lat": d.lat + 0.05, "last_lon": d.lon})
check("airborne inside 8nm reads Landing",
      tags.compute_phase(get_row(UID, LEG.id), LEG, DEP) == tags.PHASE_LANDING)

write(UID, LEG.id, always={"last_on_ground": 1})
check("on the ground after flying reads Taxi-in",
      tags.compute_phase(get_row(UID, LEG.id), LEG, DEP) == tags.PHASE_TAXI_IN)

write(UID, LEG.id, always={"last_on_ground": None, "last_lat": None, "last_lon": None,
...
write(UID, LEG.id, always={"out_scheduled": (DEP + timedelta(minutes=5)).isoformat(),
                           "out_estimated": (DEP + timedelta(minutes=5)).isoformat()})
status, _, _ = tags.compute_status(get_row(UID, LEG.id), LEG, DEP)
check("airline schedule differing from FFDO alone is NOT Delayed", status is None)

write(UID, LEG.id, always={"out_estimated": (DEP + timedelta(minutes=40)).isoformat()})
status, dep_rev, _ = tags.compute_status(get_row(UID, LEG.id), LEG, DEP)
...
write(UID, LEG.id, always={"out_estimated": DEP.isoformat()})
status, _, _ = tags.compute_status(get_row(UID, LEG.id), LEG, DEP)
...
write(UID, LEG.id, always={"out_estimated": None,
...
write(UID, L2.id, always={"airborne_seen": 1,
...
write(UID, L3.id, always={"stopped_since": stopped_at.isoformat(),
                          "airborne_seen": 0, "landed_seen": 0})
row = get_row(UID, L3.id)
...
write(UID, L3.id, always={"airborne_seen": 1, "landed_seen": 1,
...
closure.close(UID, L3, get_row(UID, L3.id), closure.SOURCE_OBSERVED,
              stopped_at, stopped_at)
check("observed close is recorded", get_row(UID, L3.id)["closed_by"] == "observed")
gate_in = (stopped_at + timedelta(minutes=4)).isoformat()
write(UID, L3.id, always={"in_actual_api": gate_in})
closure.maybe_close(UID, L3, get_row(UID, L3.id), stopped_at + timedelta(hours=1))
row = get_row(UID, L3.id)
...
write(UID, LEG.id, always={"last_lat": None, "last_lon": None, "last_on_ground": None,
                           "phase_tag": tags.PHASE_SCHEDULED, "off_actual_api": None,
                           "last_signal_at": None, "cancelled": 0})
derived = recompute_derived(get_row(UID, LEG.id), LEG, ARR + timedelta(hours=1))
...
write(UID, LEG.id, always={"last_lat": mid_lat, "last_lon": mid_lon,
                           "last_on_ground": 0, "phase_tag": tags.PHASE_IN_AIR,
                           "last_signal_at": ARR.isoformat()})
derived = recompute_derived(get_row(UID, LEG.id), LEG, ARR)
